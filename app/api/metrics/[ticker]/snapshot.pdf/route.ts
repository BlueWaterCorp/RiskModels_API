...
import { loadPeerVarianceBar } from "@/lib/portfolio/snapshot-peer-bar";
...
function singleTickerPdfKey(userId: string, ticker: string) {
  const h = createHash("sha256")
    ...
    .digest("hex");
  // _v4: fall back to sector peers when the subsector cohort is too thin.
  return generateCacheKey("risk_snapshot_pdf_ticker_v4", h);
}
...
async function buildSingleTickerPdf(
  ...
  const pdfBytes = await buildRiskSnapshotPdfBytes({
    ...
    data: core,
    peerBar: await loadPeerVarianceBar(
      ticker,
      (core.perTicker[ticker] ?? {}) as Record<string, unknown>,
    ),
  });
