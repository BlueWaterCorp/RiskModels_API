...
def _make_peer_dna_chart(dd: DDData) -> "Image.Image":
    ...
    n = len(rows)
    peerless = (n == 1)  # peer_comparison missing / empty → target-only fallback
    ...
    # tiny Sector/Subsector slivers are still readable, and a small banner
    ...
    if peerless:
        ...
        ax.text(
            0.99, 0.97,
            "Peer analytics unavailable\n(fallback: target-only)",
            transform=ax.transAxes, ha="right", va="top",
            fontsize=7, color=SLATE, style="italic",
            bbox=dict(boxstyle="round,pad=0.4",
                      facecolor="#fef3c7", edgecolor="#fcd34d", linewidth=0.8),
            zorder=15,
        )
    ...
    for lab, col in zip(_labels, (LAYER_COLORS["mkt"], LAYER_COLORS["sec"],
    ...
    # Skip the xlabel in peerless mode — the legend (with inline percentages)
    # plus the axis tick formatter already convey "Annualized σ; segments = σ ×
    # L3 explained-risk share", and leaving it in collides with the legend in
    # the short single-bar layout.
    if not peerless:
        ax.set_xlabel(
