...
describe("skill-inference v1.1", () => {
  ...
  it("forbids a universe-scoped multiplicity badge on a single-name answer", () => {
    ...
    //
    // Deliberately no discovery count here, and the reason got sharper twice.
    //
    // "Zero names survive search adjustment" was true at 2026-08-07 and false
    // at some earlier dates, so it was never a standing fact. Then the count of
    // exceptions moved too: three of 24 annual dates rejected at nominal 5%,
    // but under a pre-registered robustness criterion — a plausible common
    // shift to the factor loadings, |c| <= 0.04 — 2014 walks from 0.0495 to
    // 0.0580 and stops rejecting. Two remain, and both carry an unbounded
    // survivorship caveat, so there is currently no date with a clean
    // familywise-supported claim.
    //
    // A date-specific count in a test comment becomes a standing fact the first
