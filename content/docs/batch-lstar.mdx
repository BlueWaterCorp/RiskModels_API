...
- **`threshold`** — marginal-ER bar for the L1/L2/L3 pick. Omit for the materialized engine selection (including GBM). Any explicit value invokes the legacy threshold rule.
- **`format`** —
...
- **[Methodology](/docs/methodology)** — the L\* selection rule, the canonical selector and the legacy 1% threshold.
