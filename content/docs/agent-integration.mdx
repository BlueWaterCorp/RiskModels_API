...
1. Use the RiskModels MCP server for discovery: `riskmodels_list_endpoints`,
   `riskmodels_get_capability`, `riskmodels_get_schema` (see mcp-server/README.md).
...
The `args` path must point to `mcp-server/dist/index.js` — the compiled output. If `dist/` does not exist, run the build command above.
