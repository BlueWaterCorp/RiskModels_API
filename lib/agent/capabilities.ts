...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    description:
      "Cohort residual statistics over a date range, one series per cohort. This is the demeaning endpoint: request residual_mean at the level your residual is defined against (sector residuals demean within sector cohorts) and subtract it. Panel runs 2000-01-03 to present, though full factor richness begins around 2006. Each cohort reports proxied_fraction — the share of returned days whose factor came from a substitute instrument — because two sector cohorts are majority-proxied over long windows and a chart that hides that shows partly a different basket. Public scope is SPY + the 11 GICS sector SPDRs. linked_beta_se is a CONDITIONAL, homoskedastic model SE: it assumes iid residuals, so it is understated for daily returns, and it is unreliable wherever the rolling window is partial (early history). It is not a total-uncertainty measure — do not use it alone for confidence intervals.",
  ...
  },
  {
    ...
    description:
      "The addressable cohorts, their parent links, the variable catalogue, and the interpretation notes that govern correct use — including the no-intercept contract read directly from the store. Free discovery step; call this before /api/cohorts to learn what may be requested and what the numbers mean. linked_beta_se is a CONDITIONAL, homoskedastic model SE: it assumes iid residuals, so it is understated for daily returns, and it is unreliable wherever the rolling window is partial (early history). It is not a total-uncertainty measure — do not use it alone for confidence intervals.",
