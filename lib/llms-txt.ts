...
- "What's AAPL's ROE / WACC / economic profit / cost of capital?" → \`get_fundamentals\` (PIT quarterly, derived-only — never forecasts or analyst fields).

## Fundamentals — PIT quarterly, derived analytics only

- **\`GET /api/fundamentals/{ticker}?periods=N&as_of=YYYY-MM-DD\`** — point-in-time quarterly fundamentals for one ticker: TTM profitability (\`roe_ttm\`, \`roa_ttm\`, \`fcf_margin\`), \`leverage_ratio\`, ERM3 cascade betas, and the cost-of-capital layer (\`cost_of_equity\`, \`wacc\`, \`economic_profit\`, six-tenor \`rf_rate\`). ERP is caller-supplied via \`?erp=\`, tenor via \`?rf_tenor=\` (default 10y), or \`?grid=true\` for the erp×tenor sensitivity table. Rows are visible **iff \`filed_date <= as_of\`** — never "latest". **Realized historical only** — no forecasts, no analyst targets, no buy/sell signals. **Derived-only licensing:** raw vendor line items (revenue, net income, EPS, balance-sheet levels) are not returned; \`market_cap\` is a current snapshot, not per-quarter PIT. Per-symbol, per-call (no batch variant). **$0.005/call.** Guide: https://riskmodels.app/docs/fundamentals
