...
- **Python SDK `get_peers()` + `peers-v1.json`** — Completes G.43: `RiskModelsClient.get_peers()`, `PeerGroupProxy.from_ticker` via authenticated `GET /peers` (HTTPS only), MCP `riskmodels_get_peers`, schema `peers-v1.json` (referenced since #302 but missing from the tree).
