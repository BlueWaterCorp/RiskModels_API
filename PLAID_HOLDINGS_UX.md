...
**What is "the same account"?** For web signup and `POST /api/auth/provision`, your account is identified by **email**. You sign in at riskmodels.net with that email (and password). Your API key and any Plaid connections you make while signed in belong to that email-based account.
...
- [Authentication Guide](AUTHENTICATION_GUIDE.md) — Bearer tokens, OAuth2, scopes
- [API Documentation](README_API.md) — Core endpoints and concepts
- [OpenAPI Spec](OPENAPI_SPEC.yaml) — Full schema for `/api/plaid/holdings`
